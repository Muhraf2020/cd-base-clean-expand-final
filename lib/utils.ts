/**
 * Utility functions for the DClin
 */
...
/**
 * Generate SEO-friendly slug from text (basic version)
 */
...
/**
 * Generate SEO-friendly slug from clinic name, city, and state
 * Format: clinic-name-city-state
 * This creates more unique and SEO-friendly slugs than the basic generateSlug
 */
export function generateClinicSlug(
  ...
  // Combine name, city, and state for uniqueness and SEO
  const parts = [name];
  ...
  return combined
    ...
    .substring(0, 100); // Limit length for database and URL constraints
}

/**
 * Generate unique slug by appending number if collision exists
 * Used to ensure slug uniqueness across all clinics
 */
